...
A single run of the tool produces one P-Q flexibility area, saved in `results/` together with simulations data in JLD format. To quantify the impact of voltage unbalance limits, phase coordination constraints or other parameters, the tool has to be run multiple times and its outputs should be compared.
